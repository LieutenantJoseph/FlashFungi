...
// Main authenticated application component handling routing and state management

// React utilities
const h = React.createElement;

// User Profile Management Hook (for user progress)
function useUserProfile(authUser, getAuthToken) {
    ...
    const loadUserProgress = React.useCallback(async () => {
        ...
        try {
            const response = await fetch(`${window.API_BASE}/user-progress-api?userId=${authUser.id}`, {
                headers: {
    ...
    const saveProgress = React.useCallback(async (progressData) => {
        ...
        try {
            const response = await fetch(`${window.API_BASE}/user-progress-api`, {
                method: 'POST',
                headers: { 
...
function AuthenticatedApp() {
    ...
    // Get auth context from the auth hook
    ...
    React.useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const specimensResponse = await fetch(`${window.SUPABASE_URL}/rest/v1/specimens?select=*&order=created_at.desc`, {
                    headers: {
                ...
                const hintsResponse = await fetch(`${window.SUPABASE_URL}/rest/v1/species_hints?select=*`, {
                    headers: {
                        'apikey': window.SUPABASE_ANON_KEY,
                        'Authorization': `Bearer ${window.SUPABASE_ANON_KEY}`
                    }
                ...
                const guidesResponse = await fetch(`${window.SUPABASE_URL}/rest/v1/field_guides?select=*`, {
                    headers: {
                        'apikey': window.SUPABASE_ANON_KEY,
                        'Authorization': `Bearer ${window.SUPABASE_ANON_KEY}`
                    }
...
// Export main component to window for global access
